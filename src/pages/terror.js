var filmes = [
    {
        titulo: 'A ORFÃ ',
        conteudo: 'Terror, Suspense',
        dataLancamento: '4 de setembro de 2009 (2h 03min)',
        direcao: 'Jaume Collet-Serra',
        sinopse: 'Kate (Vera Farmiga) e John Coleman (Peter Sarsgaard) ficam arrasados devido a um trágico aborto. Apesar de já ter dois filhos, Daniel (Jimmy Bennett) e a surda muda Maxime (Aryana Engineer), o casal decide adotar uma criança. Durante uma visita a um orfanato, os dois se encantam pela pequena Esther (Isabelle Fuhrman)de nove anos e optam rapidamente por sua adoção. O que eles não sabiam é que estranhos acontecimentos fazem parte do histórico da menina que passa a se tornar, dia após dia, mais misteriosa. Intrigada, Kate desconfia que Esther não é quem aparenta ser, mas devido ao seu passado de alcoolismo tem dificuldades de provar sua teoria.',
        linkImagem: 'Terror/terrororfa.jpg',
        dataTarget: 'trailer_o_acordo',
        linkFilme: ''
    },{
        titulo: 'O CASEIRO ',
        conteudo: 'Terror, Suspense',
        dataLancamento: '23 de junho de 2016(1h 43min)',
        direcao: 'Julio Santi',
        sinopse: 'Davi, um cético professor de psicologia, é famoso por escrever um livro que explica aparições sobrenaturais através da psicanálise. Após anos sem atender pacientes, ele investiga o caso de uma garota assombrada por fantasma de um caseiro.',
        linkImagem: 'Terror/terrorcaseiro.jpg',
        dataTarget: 'trailer_logan',
        linkFilme: ''
    },{
        titulo: 'A FREIRA ',
        conteudo: 'Terror',
        dataLancamento: '6 de setembro de 2018 (1h 37min)',
        direcao: 'Corin Hardy',
        sinopse: 'Presa em um convento na Romênia, uma freira comete suicídio. Para investigar o caso, o Vaticano envia um padre atormentado e uma noviça prestes a se tornar freira. Arriscando suas vidas, a fé e até suas almas, os dois descobrem um segredo profano e se confrontam com uma força do mal que toma a forma de uma freira demoníaca e transforma o convento num campo de batalha.',
        linkImagem: 'Terror/afreira.jpg',
        dataTarget: 'trailer_desejo_de_matar',
        linkFilme: ''
    },{
        titulo: 'A DAMA DO ESPELHO ',
        conteudo: 'Terror',
        dataLancamento: '10 de Setembro de 2015 (1h 32 min)',
        direcao: 'Svyatoslav Podgayevskiy',
        sinopse: 'Diz a lenda que qualquer espelho pode tornar-se um portal para o mundo dos mortos. O ritual mais antigo conhecido pela humanidade é a convocação da Rainha de Espadas. Quatro adolescentes decidem chamar a Rainha de Espadas como uma brincadeira, mas eles não podem sequer imaginar os horrores que condenavam a si mesmos e seus entes queridos com isso. A entidade do mal não vai parar até que ela receba suas almas...',
        linkImagem: 'Terror/damaespelho.jpg',
        dataTarget: 'trailer_o_passageiro',
        linkFilme: ''
    },{
        titulo: 'ELOISE ',
        conteudo: 'Terror, Suspens',
        dataLancamento: '15 de Julho 2018 (1h 40min)',
        direcao: 'Patrick Hughes (II)',
        sinopse: 'Jacob (Chase Crawford) descobre que poderia ganhar uma grande herança, caso encontre o certificado de morte de um de seus parentes. Então, ele e mais três amigos (Eliza Dushku, Brandon T. Jackson e PJ Byrne) invadem o asilo abandonado, conhecido como Eloise, a fim de encontrar o documento. Só que uma vez lá dentro, o que vieram procurar se torna a menor de suas preocupações, quando se deparam com um local assombrado por memórias obscuras e terríveis, que eles gostariam nunca ter revelado.',
        linkImagem: 'Terror/eloise.jpg',
        dataTarget: 'trailer_dupla_explosiva',
        linkFilme: ''
    },{
        titulo: 'A MALDIÇÃO DA FLORESTA ',
        conteudo: 'Terror',
        dataLancamento: '6 de outubro de 2016 (1h 37min)',
        direcao: 'Martin Guigui',
        sinopse: 'Por questões econômicas, o conservacionista Adam Hitchens (Joseph Mawle)aceita um emprego em uma remota floresta na Irlanda, esperando encotnrar uma vida nova para ele, sua esposa Clare (Bojana Novakovic) e seu filho recém-nascido. No entanto, ao chegarem na casa nova,um agricultor com fortes crenças na tradição local o alerta para parar de mexer naquelas terras sagradas. Quando alguém invade a casa dos Hitchens a noite, querendo fazer mal a criança, e a polícia não descobre quem fez isso, o casal vai ter que lutar para proteger seu filho e sobreviver diante de misteriosos seresque habitam aquele bosque.',
        linkImagem: 'Terror/maidcaofloresta.jpg',
        dataTarget: 'trailer_alem_da_escuridao',
        linkFilme: ''
    },{
        titulo: 'O RASTRO ',
        conteudo: 'Terror, Suspense',
        dataLancamento: '18 de maio de 2017 (1h 50min)',
        direcao: 'Martin Campbell',
        sinopse: 'João Rocha (Rafael Cardoso), um jovem e talentoso médico em ascensão, acaba encarregado de uma tarefa ingrata: supervisionar a transferência de pacientes quando um hospital público da cidade do Rio de Janeiro é fechado por falta de verba.Quando tudo parece correr dentro da normalidade, uma das pacientes, criança, desaparece no meio da noite, levando João para uma jornada num mundo obscuro e perigoso.',
        linkImagem: 'Terror/orastro.jpg',
        dataTarget: 'trailer_the_forigner',
        linkFilme: ''
    },{
        titulo: 'A REVOLTA DAS BRUXAS ',
        conteudo: 'Terror',
        dataLancamento: '3 de março de 2016 (1h 33min)',
        direcao: 'Robert Eggers',
        sinopse: 'Nova Inglaterra, década de 1630. O casal William e Katherine leva uma vida cristã com suas cinco crianças em uma comunidade extremamente religiosa, até serem expulsos do local por sua fé diferente daquela permitida pelas autoridades. A família passa a morar num local isolado, à beira do bosque, sofrendo com a escassez de comida. Um dia, o bebê recém-nascido desaparece. Teria sido devorado por um lobo? Sequestrado por uma bruxa? Enquanto buscam respostas à pergunta, cada membro da família enfrenta seus piores medos e seu lado mais condenável.',
        linkImagem: 'Terror/revoltabruxas.jpg',
        dataTarget: 'trailer_linha_de_acao',
        linkFilme: ''
    }
];

window.onload = IndexJS.carregarFilmes(filmes);